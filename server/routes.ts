...
export async function registerRoutes(
  ...
  app.post('/api/mcp/invoke', async (req, res) => {
    try {
      ...
      const mcpToolName = MCP_TOOL_MAPPING[tool] || tool;
      
      console.log(`[MCP] Invoking tool: ${tool} -> ${mcpToolName}`);
      ...
      const response = mockResponses[tool] || { success: true, message: `Tool ${tool} executed` };
